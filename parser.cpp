...
namespace parsers 
{
	std::string line_parser(const std::string& line, std::string pars_symbol_start, 
										  std::string pars_sybol_end, int to_parse) 
	{
	...
	int detect_condition_parser(std::string& condition_data, std::string& read_line, int key_control, int key_length, 
								bool eq_state) 
	{
		...
		if (key_control != -1) 
		{
			...
			bool ref_ccparser = condition_control_parser(str_utils::ltrim(condition_data_ltrimmed.substr(def_key_ref.length(), 
														condition_data_ltrimmed.length() - def_key_ref.length())), eq_state, 2);
	...
	void main_parser(const std::string& file_path, const std::string& argument) 
	{
		...
		while(std::getline(enmfilef, read_line)) 
		{
			...
			{
				run::error_control(ERROR_VAR_NOT_EXISTS, line_counter, read_line.substr(var_not_exists_error.length(), 
																 read_line.length() - var_not_exists_error.length()));
			...
			{ 
				...
				if (deps_parser_result == -3) 
				{
					...
					controllers_set_true();
					std::cout << color_codes_parser.color_warning_s << UP_TO_DATE_0 << read_line 
							  << UP_TO_DATE_1 << color_codes_parser.color_warning_e << "\n";
			...
			condition_key_for_length = IF_KEY;
			detect_condition_parser_result = detect_condition_parser(condition_data, read_line, //eq
												key_control, condition_key_for_length.length(), false);
			...
			condition_key_for_length = ELIF_KEY;                                                                                                
			detect_condition_parser_result = detect_condition_parser(condition_data, read_line, key_control,   //eq
																	 		condition_key_for_length.length(), true);
